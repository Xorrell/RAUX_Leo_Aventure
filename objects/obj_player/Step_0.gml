/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 0235C350
/// @DnDArgument : "var" "obj_mngr_hp.player_hp"
/// @DnDArgument : "op" "2"
if(obj_mngr_hp.player_hp > 0){	/// @DnDAction : YoYo Games.Gamepad.Get_Gamepad_Device_Count
	/// @DnDVersion : 1
	/// @DnDHash : 44D3AFF3
	/// @DnDParent : 0235C350
	/// @DnDArgument : "var" "gamepadcount"
	var l44D3AFF3_0 = gamepad_get_device_count();var l44D3AFF3_1 = 0;for(var l44D3AFF3_2 = 0; l44D3AFF3_2 < l44D3AFF3_0; ++l44D3AFF3_2) {	if(gamepad_is_connected(l44D3AFF3_2)) { ++l44D3AFF3_1; }}gamepadcount = l44D3AFF3_1;

	/// @DnDAction : YoYo Games.Miscellaneous.Debug_Show_Message
	/// @DnDVersion : 1
	/// @DnDHash : 2494A39D
	/// @DnDDisabled : 1
	/// @DnDParent : 0235C350
	/// @DnDArgument : "msg" "gamepadcount"


	/// @DnDAction : YoYo Games.Gamepad.Get_Gamepad_Connected
	/// @DnDVersion : 1
	/// @DnDHash : 7FD21E3C
	/// @DnDParent : 0235C350
	/// @DnDArgument : "var" "gamepadconnected"
	gamepadconnected = gamepad_is_connected(0);

	/// @DnDAction : YoYo Games.Miscellaneous.Debug_Show_Message
	/// @DnDVersion : 1
	/// @DnDHash : 4AF52FE7
	/// @DnDDisabled : 1
	/// @DnDParent : 0235C350
	/// @DnDArgument : "msg" "gamepadconnected"


	/// @DnDAction : YoYo Games.Common.If_Variable
	/// @DnDVersion : 1
	/// @DnDHash : 367A0437
	/// @DnDParent : 0235C350
	/// @DnDArgument : "var" "gamepadcount"
	/// @DnDArgument : "value" "1"
	if(gamepadcount == 1){	/// @DnDAction : YoYo Games.Gamepad.Set_Gamepad_Axis_Deadzone
		/// @DnDVersion : 1
		/// @DnDHash : 5661E311
		/// @DnDParent : 367A0437
		/// @DnDArgument : "gp" "1"
		gamepad_set_axis_deadzone(1, 0.2);
	
		/// @DnDAction : YoYo Games.Gamepad.If_Gamepad_Button_Down
		/// @DnDVersion : 1.1
		/// @DnDHash : 6121B105
		/// @DnDParent : 367A0437
		/// @DnDArgument : "gp" "1"
		/// @DnDArgument : "btn" "gp_stickl"
		var l6121B105_0 = 1;var l6121B105_1 = gp_stickl;if(gamepad_is_connected(l6121B105_0) && (gamepad_button_check(l6121B105_0, l6121B105_1) || gamepad_button_check_released(l6121B105_0, l6121B105_1))){	/// @DnDAction : YoYo Games.Gamepad.If_Gamepad_Button_Pressed
			/// @DnDVersion : 1.1
			/// @DnDHash : 65196961
			/// @DnDParent : 6121B105
			/// @DnDArgument : "btn" "gp_face2"
			var l65196961_0 = 0;var l65196961_1 = gp_face2;if(gamepad_is_connected(l65196961_0) && gamepad_button_check_pressed(l65196961_0, l65196961_1)){	/// @DnDAction : YoYo Games.Common.If_Variable
				/// @DnDVersion : 1
				/// @DnDHash : 76BADA3F
				/// @DnDParent : 65196961
				/// @DnDArgument : "var" "atk_cooldown"
				/// @DnDArgument : "not" "1"
				/// @DnDArgument : "op" "2"
				if(!(atk_cooldown > 0)){	/// @DnDAction : YoYo Games.Common.Variable
					/// @DnDVersion : 1
					/// @DnDHash : 2154651D
					/// @DnDParent : 76BADA3F
					/// @DnDArgument : "expr" "15"
					/// @DnDArgument : "var" "atk_cooldown"
					atk_cooldown = 15;
				
					/// @DnDAction : YoYo Games.Common.If_Variable
					/// @DnDVersion : 1
					/// @DnDHash : 56542E1F
					/// @DnDParent : 76BADA3F
					/// @DnDArgument : "var" "facing"
					/// @DnDArgument : "value" ""North""
					if(facing == "North"){	/// @DnDAction : YoYo Games.Instances.Create_Instance
						/// @DnDVersion : 1
						/// @DnDHash : 03CB8975
						/// @DnDParent : 56542E1F
						/// @DnDArgument : "xpos_relative" "1"
						/// @DnDArgument : "ypos" "-24"
						/// @DnDArgument : "ypos_relative" "1"
						/// @DnDArgument : "objectid" "obj_attackswing"
						/// @DnDSaveInfo : "objectid" "obj_attackswing"
						instance_create_layer(x + 0, y + -24, "Instances", obj_attackswing);
					
						/// @DnDAction : YoYo Games.Instances.Sprite_Rotate
						/// @DnDVersion : 1
						/// @DnDHash : 3A8847C2
						/// @DnDApplyTo : {obj_attackswing}
						/// @DnDParent : 56542E1F
						with(obj_attackswing) image_angle = 0;}
				
					/// @DnDAction : YoYo Games.Common.If_Variable
					/// @DnDVersion : 1
					/// @DnDHash : 4707EEC5
					/// @DnDParent : 76BADA3F
					/// @DnDArgument : "var" "facing"
					/// @DnDArgument : "value" ""Left""
					if(facing == "Left"){	/// @DnDAction : YoYo Games.Instances.Create_Instance
						/// @DnDVersion : 1
						/// @DnDHash : 1194A2B8
						/// @DnDParent : 4707EEC5
						/// @DnDArgument : "xpos" "-24"
						/// @DnDArgument : "xpos_relative" "1"
						/// @DnDArgument : "ypos_relative" "1"
						/// @DnDArgument : "objectid" "obj_attackswing"
						/// @DnDSaveInfo : "objectid" "obj_attackswing"
						instance_create_layer(x + -24, y + 0, "Instances", obj_attackswing);
					
						/// @DnDAction : YoYo Games.Instances.Sprite_Rotate
						/// @DnDVersion : 1
						/// @DnDHash : 77CCBE59
						/// @DnDApplyTo : {obj_attackswing}
						/// @DnDParent : 4707EEC5
						/// @DnDArgument : "angle" "90"
						with(obj_attackswing) image_angle = 90;}
				
					/// @DnDAction : YoYo Games.Common.If_Variable
					/// @DnDVersion : 1
					/// @DnDHash : 34F6E065
					/// @DnDParent : 76BADA3F
					/// @DnDArgument : "var" "facing"
					/// @DnDArgument : "value" ""Right""
					if(facing == "Right"){	/// @DnDAction : YoYo Games.Instances.Create_Instance
						/// @DnDVersion : 1
						/// @DnDHash : 2CB03C8B
						/// @DnDParent : 34F6E065
						/// @DnDArgument : "xpos" "24"
						/// @DnDArgument : "xpos_relative" "1"
						/// @DnDArgument : "ypos_relative" "1"
						/// @DnDArgument : "objectid" "obj_attackswing"
						/// @DnDSaveInfo : "objectid" "obj_attackswing"
						instance_create_layer(x + 24, y + 0, "Instances", obj_attackswing);
					
						/// @DnDAction : YoYo Games.Instances.Sprite_Rotate
						/// @DnDVersion : 1
						/// @DnDHash : 2E8822AF
						/// @DnDApplyTo : {obj_attackswing}
						/// @DnDParent : 34F6E065
						/// @DnDArgument : "angle" "-90"
						with(obj_attackswing) image_angle = -90;}
				
					/// @DnDAction : YoYo Games.Common.If_Variable
					/// @DnDVersion : 1
					/// @DnDHash : 44CA7E79
					/// @DnDParent : 76BADA3F
					/// @DnDArgument : "var" "facing"
					/// @DnDArgument : "value" ""South""
					if(facing == "South"){	/// @DnDAction : YoYo Games.Instances.Create_Instance
						/// @DnDVersion : 1
						/// @DnDHash : 008DFA64
						/// @DnDParent : 44CA7E79
						/// @DnDArgument : "xpos_relative" "1"
						/// @DnDArgument : "ypos" "24"
						/// @DnDArgument : "ypos_relative" "1"
						/// @DnDArgument : "objectid" "obj_attackswing"
						/// @DnDSaveInfo : "objectid" "obj_attackswing"
						instance_create_layer(x + 0, y + 24, "Instances", obj_attackswing);
					
						/// @DnDAction : YoYo Games.Instances.Sprite_Rotate
						/// @DnDVersion : 1
						/// @DnDHash : 67E95A81
						/// @DnDApplyTo : {obj_attackswing}
						/// @DnDParent : 44CA7E79
						/// @DnDArgument : "angle" "180"
						with(obj_attackswing) image_angle = 180;}}}}}

	/// @DnDAction : YoYo Games.Common.If_Variable
	/// @DnDVersion : 1
	/// @DnDHash : 383CAE3E
	/// @DnDParent : 0235C350
	/// @DnDArgument : "var" "player_teleporting or player_attacksp"
	/// @DnDArgument : "not" "1"
	/// @DnDArgument : "value" "true"
	if(!(player_teleporting or player_attacksp == true)){	/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Down
		/// @DnDVersion : 1
		/// @DnDHash : 762BBEEF
		/// @DnDParent : 383CAE3E
		/// @DnDArgument : "key" "ord("Z")"
		var l762BBEEF_0;l762BBEEF_0 = keyboard_check(ord("Z"));if (l762BBEEF_0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 48674B5D
			/// @DnDParent : 762BBEEF
			/// @DnDArgument : "expr" "-6"
			/// @DnDArgument : "var" "vmove"
			vmove = -6;
		
			/// @DnDAction : YoYo Games.Movement.Set_Speed
			/// @DnDVersion : 1
			/// @DnDHash : 42458B32
			/// @DnDParent : 762BBEEF
			/// @DnDArgument : "speed" "vmove"
			/// @DnDArgument : "type" "2"
			vspeed = vmove;
		
			/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 1392F839
			/// @DnDParent : 762BBEEF
			/// @DnDArgument : "expr" ""North""
			/// @DnDArgument : "var" "facing"
			facing = "North";}
	
		/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Down
		/// @DnDVersion : 1
		/// @DnDHash : 70AE076A
		/// @DnDParent : 383CAE3E
		/// @DnDArgument : "key" "ord("Q")"
		var l70AE076A_0;l70AE076A_0 = keyboard_check(ord("Q"));if (l70AE076A_0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 7480A672
			/// @DnDParent : 70AE076A
			/// @DnDArgument : "expr" "-6"
			/// @DnDArgument : "var" "hmove"
			hmove = -6;
		
			/// @DnDAction : YoYo Games.Movement.Set_Speed
			/// @DnDVersion : 1
			/// @DnDHash : 16249C20
			/// @DnDParent : 70AE076A
			/// @DnDArgument : "speed" "hmove"
			/// @DnDArgument : "type" "1"
			hspeed = hmove;
		
			/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 25C09F54
			/// @DnDParent : 70AE076A
			/// @DnDArgument : "expr" ""Left""
			/// @DnDArgument : "var" "facing"
			facing = "Left";}
	
		/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Down
		/// @DnDVersion : 1
		/// @DnDHash : 0DC69A36
		/// @DnDParent : 383CAE3E
		/// @DnDArgument : "key" "ord("D")"
		var l0DC69A36_0;l0DC69A36_0 = keyboard_check(ord("D"));if (l0DC69A36_0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 08D6582E
			/// @DnDParent : 0DC69A36
			/// @DnDArgument : "expr" "6"
			/// @DnDArgument : "var" "hmove"
			hmove = 6;
		
			/// @DnDAction : YoYo Games.Movement.Set_Speed
			/// @DnDVersion : 1
			/// @DnDHash : 35590121
			/// @DnDParent : 0DC69A36
			/// @DnDArgument : "speed" "hmove"
			/// @DnDArgument : "type" "1"
			hspeed = hmove;
		
			/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 2A77C80A
			/// @DnDParent : 0DC69A36
			/// @DnDArgument : "expr" ""Right""
			/// @DnDArgument : "var" "facing"
			facing = "Right";}
	
		/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Down
		/// @DnDVersion : 1
		/// @DnDHash : 0B8C506B
		/// @DnDParent : 383CAE3E
		/// @DnDArgument : "key" "ord("S")"
		var l0B8C506B_0;l0B8C506B_0 = keyboard_check(ord("S"));if (l0B8C506B_0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 435B10A6
			/// @DnDParent : 0B8C506B
			/// @DnDArgument : "expr" "6"
			/// @DnDArgument : "var" "vmove"
			vmove = 6;
		
			/// @DnDAction : YoYo Games.Movement.Set_Speed
			/// @DnDVersion : 1
			/// @DnDHash : 7F217DA8
			/// @DnDParent : 0B8C506B
			/// @DnDArgument : "speed" "vmove"
			/// @DnDArgument : "type" "2"
			vspeed = vmove;
		
			/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 6EABF05C
			/// @DnDParent : 0B8C506B
			/// @DnDArgument : "expr" ""South""
			/// @DnDArgument : "var" "facing"
			facing = "South";
		
			/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
			/// @DnDVersion : 1
			/// @DnDHash : 34E67125
			/// @DnDDisabled : 1
			/// @DnDParent : 0B8C506B
			/// @DnDArgument : "obj" "obj_sword_atksp_neu"
			/// @DnDArgument : "not" "1"
			/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
			/// @DnDAction : YoYo Games.Instances.Set_Sprite
			/// @DnDVersion : 1
			/// @DnDHash : 3B7712CD
			/// @DnDDisabled : 1
			/// @DnDParent : 34E67125
			/// @DnDArgument : "spriteind" "spr_player_idle_south"
			/// @DnDSaveInfo : "spriteind" "spr_player_idle_south"
			
			
			/// @DnDAction : YoYo Games.Miscellaneous.Debug_Show_Message
			/// @DnDVersion : 1
			/// @DnDHash : 1C159993
			/// @DnDDisabled : 1
			/// @DnDParent : 34E67125
			/// @DnDArgument : "msg" ""hello""
			
			
			/// @DnDAction : YoYo Games.Instances.Set_Sprite
			/// @DnDVersion : 1
			/// @DnDHash : 0D2E76D1
			/// @DnDParent : 34E67125
			/// @DnDArgument : "spriteind" "spr_player_walk_south"
			/// @DnDSaveInfo : "spriteind" "spr_player_walk_south"
			sprite_index = spr_player_walk_south;
			image_index = 0;}}

	/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Released
	/// @DnDVersion : 1
	/// @DnDHash : 7FE4DEBF
	/// @DnDParent : 0235C350
	/// @DnDArgument : "key" "ord("Q") or ord("D")"
	var l7FE4DEBF_0;l7FE4DEBF_0 = keyboard_check_released(ord("Q") or ord("D"));if (l7FE4DEBF_0){	/// @DnDAction : YoYo Games.Common.Variable
		/// @DnDVersion : 1
		/// @DnDHash : 6B4E091C
		/// @DnDParent : 7FE4DEBF
		/// @DnDArgument : "var" "hmove"
		hmove = 0;
	
		/// @DnDAction : YoYo Games.Movement.Set_Speed
		/// @DnDVersion : 1
		/// @DnDHash : 014D5EF9
		/// @DnDParent : 7FE4DEBF
		/// @DnDArgument : "speed" "hmove"
		/// @DnDArgument : "type" "1"
		hspeed = hmove;}

	/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Released
	/// @DnDVersion : 1
	/// @DnDHash : 59266B89
	/// @DnDParent : 0235C350
	/// @DnDArgument : "key" "ord("Z") or ord("S")"
	var l59266B89_0;l59266B89_0 = keyboard_check_released(ord("Z") or ord("S"));if (l59266B89_0){	/// @DnDAction : YoYo Games.Common.Variable
		/// @DnDVersion : 1
		/// @DnDHash : 3C75348E
		/// @DnDParent : 59266B89
		/// @DnDArgument : "var" "vmove"
		vmove = 0;
	
		/// @DnDAction : YoYo Games.Movement.Set_Speed
		/// @DnDVersion : 1
		/// @DnDHash : 5F55ACF6
		/// @DnDParent : 59266B89
		/// @DnDArgument : "speed" "vmove"
		/// @DnDArgument : "type" "2"
		vspeed = vmove;}

	/// @DnDAction : YoYo Games.Common.If_Variable
	/// @DnDVersion : 1
	/// @DnDHash : 178A378E
	/// @DnDParent : 0235C350
	/// @DnDArgument : "var" "vmove and hmove"
	/// @DnDArgument : "not" "1"
	if(!(vmove and hmove == 0)){	/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 0CFECC6A
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "vmove"
		/// @DnDArgument : "op" "2"
		if(vmove > 0){	/// @DnDAction : YoYo Games.Miscellaneous.Debug_Show_Message
			/// @DnDVersion : 1
			/// @DnDHash : 25335216
			/// @DnDParent : 0CFECC6A
			/// @DnDArgument : "msg" "vmove"
			show_debug_message(string(vmove));
		
			/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Pressed
			/// @DnDVersion : 1
			/// @DnDHash : 24880A8D
			/// @DnDParent : 0CFECC6A
			/// @DnDArgument : "key" "ord("S")"
			var l24880A8D_0;l24880A8D_0 = keyboard_check_pressed(ord("S"));if (l24880A8D_0){	/// @DnDAction : YoYo Games.Common.If_Variable
				/// @DnDVersion : 1
				/// @DnDHash : 6AC0F7E8
				/// @DnDParent : 24880A8D
				/// @DnDArgument : "var" "vmove"
				/// @DnDArgument : "not" "1"
				if(!(vmove == 0)){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
					/// @DnDVersion : 1
					/// @DnDHash : 77C4D937
					/// @DnDParent : 6AC0F7E8
					/// @DnDArgument : "obj" "obj_sword_atksp_neu"
					/// @DnDArgument : "not" "1"
					/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
					var l77C4D937_0 = false;l77C4D937_0 = instance_exists(obj_sword_atksp_neu);if(!l77C4D937_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
						/// @DnDVersion : 1
						/// @DnDHash : 5D9D2B21
						/// @DnDDisabled : 1
						/// @DnDParent : 77C4D937
						/// @DnDArgument : "spriteind" "spr_player_walk_south"
						/// @DnDSaveInfo : "spriteind" "spr_player_walk_south"}}}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 43A5AA95
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "vmove"
		/// @DnDArgument : "op" "1"
		if(vmove < 0){	/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Pressed
			/// @DnDVersion : 1
			/// @DnDHash : 1D68F068
			/// @DnDParent : 43A5AA95
			/// @DnDArgument : "key" "ord("Z")"
			var l1D68F068_0;l1D68F068_0 = keyboard_check_pressed(ord("Z"));if (l1D68F068_0){	/// @DnDAction : YoYo Games.Common.If_Variable
				/// @DnDVersion : 1
				/// @DnDHash : 33CE03D3
				/// @DnDParent : 1D68F068
				/// @DnDArgument : "var" "vmove"
				/// @DnDArgument : "not" "1"
				if(!(vmove == 0)){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
					/// @DnDVersion : 1
					/// @DnDHash : 33EC7311
					/// @DnDParent : 33CE03D3
					/// @DnDArgument : "obj" "obj_sword_atksp_neu"
					/// @DnDArgument : "not" "1"
					/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
					var l33EC7311_0 = false;l33EC7311_0 = instance_exists(obj_sword_atksp_neu);if(!l33EC7311_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
						/// @DnDVersion : 1
						/// @DnDHash : 3DB9D9CF
						/// @DnDDisabled : 1
						/// @DnDParent : 33EC7311
						/// @DnDArgument : "spriteind" "spr_player_walk_north"
						/// @DnDSaveInfo : "spriteind" "spr_player_walk_north"}}}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 5A222902
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "hmove"
		/// @DnDArgument : "op" "1"
		if(hmove < 0){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
			/// @DnDVersion : 1
			/// @DnDHash : 679B43D8
			/// @DnDParent : 5A222902
			/// @DnDArgument : "obj" "obj_sword_atksp_neu"
			/// @DnDArgument : "not" "1"
			/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
			var l679B43D8_0 = false;l679B43D8_0 = instance_exists(obj_sword_atksp_neu);if(!l679B43D8_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
				/// @DnDVersion : 1
				/// @DnDHash : 0918E168
				/// @DnDParent : 679B43D8
				/// @DnDArgument : "spriteind" "spr_player_idle_west"
				/// @DnDSaveInfo : "spriteind" "spr_player_idle_west"
				sprite_index = spr_player_idle_west;
				image_index = 0;}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 7AC8321D
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "hmove"
		/// @DnDArgument : "op" "1"
		if(hmove < 0){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
			/// @DnDVersion : 1
			/// @DnDHash : 22204EA4
			/// @DnDParent : 7AC8321D
			/// @DnDArgument : "obj" "obj_sword_atksp_neu"
			/// @DnDArgument : "not" "1"
			/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
			var l22204EA4_0 = false;l22204EA4_0 = instance_exists(obj_sword_atksp_neu);if(!l22204EA4_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
				/// @DnDVersion : 1
				/// @DnDHash : 51F5CD95
				/// @DnDParent : 22204EA4
				/// @DnDArgument : "spriteind" "spr_player_idle_west"
				/// @DnDSaveInfo : "spriteind" "spr_player_idle_west"
				sprite_index = spr_player_idle_west;
				image_index = 0;}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 5B104165
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "vmove"
		/// @DnDArgument : "op" "2"
		if(vmove > 0){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
			/// @DnDVersion : 1
			/// @DnDHash : 6C993047
			/// @DnDParent : 5B104165
			/// @DnDArgument : "obj" "obj_sword_atksp_neu"
			/// @DnDArgument : "not" "1"
			/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
			var l6C993047_0 = false;l6C993047_0 = instance_exists(obj_sword_atksp_neu);if(!l6C993047_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
				/// @DnDVersion : 1
				/// @DnDHash : 5A9C131E
				/// @DnDParent : 6C993047
				/// @DnDArgument : "spriteind" "spr_player_idle_south"
				/// @DnDSaveInfo : "spriteind" "spr_player_idle_south"
				sprite_index = spr_player_idle_south;
				image_index = 0;}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 5E1340FD
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "vmove"
		/// @DnDArgument : "op" "1"
		if(vmove < 0){	/// @DnDAction : YoYo Games.Instances.If_Instance_Exists
			/// @DnDVersion : 1
			/// @DnDHash : 13433777
			/// @DnDParent : 5E1340FD
			/// @DnDArgument : "obj" "obj_sword_atksp_neu"
			/// @DnDArgument : "not" "1"
			/// @DnDSaveInfo : "obj" "obj_sword_atksp_neu"
			var l13433777_0 = false;l13433777_0 = instance_exists(obj_sword_atksp_neu);if(!l13433777_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
				/// @DnDVersion : 1
				/// @DnDHash : 4665B6B6
				/// @DnDParent : 13433777
				/// @DnDArgument : "spriteind" "spr_player_idle_north"
				/// @DnDSaveInfo : "spriteind" "spr_player_idle_north"
				sprite_index = spr_player_idle_north;
				image_index = 0;}}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 15171845
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "atk_cooldown"
		/// @DnDArgument : "not" "1"
		/// @DnDArgument : "op" "3"
		/// @DnDArgument : "value" "-2"
		if(!(atk_cooldown <= -2)){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 464F72B5
			/// @DnDParent : 15171845
			/// @DnDArgument : "expr" "-1"
			/// @DnDArgument : "expr_relative" "1"
			/// @DnDArgument : "var" "atk_cooldown"
			atk_cooldown += -1;
		
			/// @DnDAction : YoYo Games.Miscellaneous.Debug_Show_Message
			/// @DnDVersion : 1
			/// @DnDHash : 6925BBC9
			/// @DnDDisabled : 1
			/// @DnDParent : 15171845
			/// @DnDArgument : "msg" "atk_cooldown"}
	
		/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Down
		/// @DnDVersion : 1
		/// @DnDHash : 186C113C
		/// @DnDParent : 178A378E
		/// @DnDArgument : "key" "ord("K")"
		/// @DnDArgument : "not" "1"
		var l186C113C_0;l186C113C_0 = keyboard_check(ord("K"));if (!l186C113C_0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 4B7FB891
			/// @DnDParent : 186C113C
			/// @DnDArgument : "expr" "-2"
			/// @DnDArgument : "expr_relative" "1"
			/// @DnDArgument : "var" "spatk_charge"
			spatk_charge += -2;}
	
		/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 207B3799
		/// @DnDParent : 178A378E
		/// @DnDArgument : "var" "spatk_charge"
		/// @DnDArgument : "op" "3"
		if(spatk_charge <= 0){	/// @DnDAction : YoYo Games.Common.Variable
			/// @DnDVersion : 1
			/// @DnDHash : 012836E6
			/// @DnDParent : 207B3799
			/// @DnDArgument : "var" "spatk_charge"
			spatk_charge = 0;}
	
		/// @DnDAction : YoYo Games.Collisions.If_Object_At
		/// @DnDVersion : 1.1
		/// @DnDHash : 1CBDB79F
		/// @DnDParent : 178A378E
		/// @DnDArgument : "x_relative" "1"
		/// @DnDArgument : "y_relative" "1"
		/// @DnDArgument : "object" "obj_door"
		/// @DnDArgument : "not" "1"
		/// @DnDSaveInfo : "object" "obj_door"
		var l1CBDB79F_0 = instance_place(x + 0, y + 0, [obj_door]);if (!(l1CBDB79F_0 > 0)){	/// @DnDAction : YoYo Games.Instances.Destroy_Instance
			/// @DnDVersion : 1
			/// @DnDHash : 43ACBA66
			/// @DnDApplyTo : {obj_popup}
			/// @DnDParent : 1CBDB79F
			with(obj_popup) instance_destroy();}}}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 7BF6A06B
/// @DnDArgument : "var" "obj_mngr_hp.player_hp"
/// @DnDArgument : "op" "3"
if(obj_mngr_hp.player_hp <= 0){	/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Pressed
	/// @DnDVersion : 1
	/// @DnDHash : 122E18B0
	/// @DnDParent : 7BF6A06B
	/// @DnDArgument : "key" "vk_enter"
	var l122E18B0_0;l122E18B0_0 = keyboard_check_pressed(vk_enter);if (l122E18B0_0){	/// @DnDAction : YoYo Games.Game.Restart_Game
		/// @DnDVersion : 1
		/// @DnDHash : 598ED6FF
		/// @DnDParent : 122E18B0
		game_restart();}}

/// @DnDAction : YoYo Games.Mouse & Keyboard.If_Key_Pressed
/// @DnDVersion : 1
/// @DnDHash : 41FAE7C9
/// @DnDArgument : "key" "ord("D")"
var l41FAE7C9_0;l41FAE7C9_0 = keyboard_check_pressed(ord("D"));if (l41FAE7C9_0){	/// @DnDAction : YoYo Games.Instances.Set_Sprite
	/// @DnDVersion : 1
	/// @DnDHash : 7E30032C
	/// @DnDParent : 41FAE7C9
	/// @DnDArgument : "spriteind" "spr_player_idle_east"
	/// @DnDSaveInfo : "spriteind" "spr_player_idle_east"
	sprite_index = spr_player_idle_east;
	image_index = 0;}